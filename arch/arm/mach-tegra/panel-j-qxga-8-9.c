...
static tegra_dc_bl_output dsi_j_qxga_8_9_bl_response_curve = {
...
static struct platform_pwm_backlight_data dsi_j_qxga_8_9_bl_data = {
	...
	.dft_brightness	= 77,
	.pwm_period_ns	= 29334,
	.bl_measured    = dsi_j_qxga_8_9_bl_response_curve,
	.notify		= dsi_j_qxga_8_9_bl_notify,
	/* Only toggle backlight on fb blank notifications for disp1 */
	.check_fb	= dsi_j_qxga_8_9_check_fb,
};

static struct platform_device __maybe_unused dsi_j_qxga_8_9_bl_device = {
	.name	= "pwm-backlight",
	.id	= -1,
	.dev	= {
...
static int dsi_j_qxga_8_9_reg_get(struct device *dev)
{
	int err = 0;
...
static int dsi_j_qxga_8_9_gpio_get(void)
{
	...
	err = gpio_request(dsi_j_qxga_8_9_pdata.dsi_panel_rst_gpio,
		"panel rst");
	if (err < 0) {
	...
	err = gpio_request(dsi_j_qxga_8_9_pdata.dsi_panel_bl_pwm_gpio,
		"panel pwm");
	if (err < 0) {
		pr_err("panel backlight pwm gpio request failed\n");
		return err;
	}
	gpio_free(dsi_j_qxga_8_9_pdata.dsi_panel_bl_pwm_gpio);
...
static int dsi_j_qxga_8_9_enable(struct device *dev)
{
	...
	err = dsi_j_qxga_8_9_reg_get(dev);
	if (err < 0) {
	...
	if (gpio_is_valid(panel_of.panel_gpio[TEGRA_GPIO_RESET]))
		...
		en_panel_rst =
			dsi_j_qxga_8_9_pdata.dsi_panel_rst_gpio;
...
static int dsi_j_qxga_8_9_register_bl_dev(void)
{
	int err = 0;
	...
	if (!of_have_populated_dt() || !dc1_node ||
		...
		err = platform_device_register(&dsi_j_qxga_8_9_bl_device);
		if (err) {
...
static void dsi_j_qxga_8_9_sd_settings_init
