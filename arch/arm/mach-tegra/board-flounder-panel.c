...
static int flounder_hdmi_disable(struct device *dev)
{
	if (flounder_hdmi_reg) {
...
static void flounder_hdmi_hotplug_report(bool state)
{
...
static struct tegra_dc_out flounder_disp2_out = {
	...
	.hotplug_report = flounder_hdmi_hotplug_report,
};
...
static void flounder_panel_select(void)
{
	...
	if (panel) {
		if (panel->init_dc_out) {
			...
			flounder_disp1_out.dsi->dsi_instance = dsi_instance;
...
int __init flounder_panel_init(int board_id)
{
	int err = 0;
