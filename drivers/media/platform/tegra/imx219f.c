...
static int debug_i2c_write(void *data, u64 val)
{
	...
	if (imx219_write_reg(info->i2c_client,
		...
		dev_info(&info->i2c_client->dev, "imx219:%s failed\n", __func__);
		return -EIO;
	}
	return 0;
}
...
{
	dev_info(&info->i2c_client->dev, "%s", __func__);
...
err_out:
	dev_info(&info->i2c_client->dev, "ERROR:%s failed", __func__);
			...
			const struct i2c_device_id *id)
{
	...
	if (IS_ERR(info->mclk)) {
		dev_info(&client->dev, "%s: unable to get clock %s\n",
			__func__, mclk_name);
