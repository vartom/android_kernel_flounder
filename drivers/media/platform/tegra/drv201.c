...
void drv201_set_arc_mode(struct drv201_info *info)
{
	...
	if (err)
		dev_info(info->dev, "%s: CONTROL reg write failed\n", __func__);

	/* set the ARC RES mode */
	err = drv201_i2c_wr8(info, MODE, (sr >> 8) & 0xff);
	if (err)
		dev_info(info->dev, "%s: MODE reg write failed\n", __func__);

	/* set the VCM_FREQ */
	err = drv201_i2c_wr8(info, VCM_FREQ, sr & 0xff);
	if (err)
		dev_info(info->dev, "%s: VCM_FREQ reg write failed\n", __func__);
}
...
/*		drv201_set_arc_mode(info);
	...
	if (err < 0) {
		dev_info(info->dev, "%s err %d\n", __func__, err);
		pwr = NVC_PWR_ERR;
	}

	info->pwr_dev = pwr;
	dev_info(info->dev, "%s pwr_dev=%d\n", __func__, info->pwr_dev);
...
static int drv201_power_get(struct drv201_info *info)
{
	...
	drv201_regulator_get(info, &pw->vdd, "avdd_ca55m1_cam");
	drv201_regulator_get(info, &pw->vdd_i2c, "vdd_244501_cam");
...
{
	dev_info(info->dev, "%s:\n", __func__);
	dev_info(info->dev, "focal_length:               0x%x\n",
		info->nv_config.focal_length);
	dev_info(info->dev, "fnumber:                    0x%x\n",
		info->nv_config.fnumber);
	dev_info(info->dev, "max_aperture:               0x%x\n",
		info->nv_config.max_aperture);
	dev_info(info->dev, "pos_working_low:            %d\n",
		info->nv_config.pos_working_low);
	dev_info(info->dev, "pos_working_high:           %d\n",
		info->nv_config.pos_working_high);
	dev_info(info->dev, "pos_actual_low:             %d\n",
		info->nv_config.pos_actual_low);
	dev_info(info->dev, "pos_actual_high:            %d\n",
		info->nv_config.pos_actual_high);
	dev_info(info->dev, "slew_rate:                  0x%x\n",
		info->nv_config.slew_rate);
	dev_info(info->dev, "circle_of_confusion:        %d\n",
		info->nv_config.circle_of_confusion);
	dev_info(info->dev, "num_focuser_sets:           %d\n",
		info->nv_config.num_focuser_sets);
	dev_info(info->dev, "focuser_set[0].macro:       %d\n",
		info->nv_config.focuser_set[0].macro);
	dev_info(info->dev, "focuser_set[0].hyper:       %d\n",
		info->nv_config.focuser_set[0].hyper);
	dev_info(info->dev, "focuser_set[0].inf:         %d\n",
		info->nv_config.focuser_set[0].inf);
	dev_info(info->dev, "focuser_set[0].settle_time: %d\n",
					...
					struct nvc_param *params)
{
	if (copy_from_user(&info->cfg_usr,
		(const void __user *)(uintptr_t)params->p_value, sizeof(info->cfg_usr))) {
			dev_info(info->dev, "%s Err: copy_from_user bytes %d\n",
...
static int drv201_param_rd(
	struct drv201_info *info, unsigned long arg, bool is_compat)
{
	struct nvc_param params;
	...
	if (get_user_nvc_param(&info->i2c_client->dev, is_compat, arg, 0,
			&params, NULL)) {
		dev_info(&info->i2c_client->dev, "%s %d copy_from_user err\n",
			__func__, __LINE__);
		return -EFAULT;
	}
	switch (params.param) {
	...
	case NVC_PARAM_MAX_APERTURE:
		...
		dev_info(info->dev, "%s MAX_APERTURE: %x\n",
	...
	default:
		dev_info(info->dev, "%s unsupported parameter: %d\n",
	...
	struct drv201_info *info, unsigned long arg, bool is_compat)
{
	...
	if (get_user_nvc_param(&info->i2c_client->dev, is_compat, arg, 0,
			&params, NULL)) {
		dev_info(info->dev, "%s copy_from_user err line %d\n",
			__func__, __LINE__);
		return -EFAULT;
	}
	if (copy_from_user(&u32val,
		(const void __user *)(uintptr_t)params.p_value, sizeof(u32val))) {
		dev_info(info->dev, "%s %d copy_from_user err\n",
			__func__, __LINE__);
		return -EFAULT;
	}
	u8val = (u8)u32val;
	...
	case NVC_PARAM_RESET:
		...
		dev_info(info->dev, "%s RESET: %d\n", __func__, err);
		break;
	case NVC_PARAM_SELF_TEST:
		err = 0;
		dev_info(info->dev, "%s SELF_TEST: %d\n", __func__, err);
		break;
	default:
		dev_info(info->dev, "%s unsupported parameter: %d\n",
			__func__, params.param);
					...
					unsigned long arg)
{
	...
	case NVC_IOCTL32_PARAM_WR:
		...
		return drv201_param_wr(info, arg, true);
	case NVC_IOCTL32_PARAM_RD:
		drv201_pm_dev_wr(info, NVC_PWR_ON);
		return drv201_param_rd(info, arg, true);
#endif
	case NVC_IOCTL_PARAM_WR:
		drv201_pm_dev_wr(info, NVC_PWR_ON);
		return drv201_param_wr(info, arg, false);
	case NVC_IOCTL_PARAM_RD:
		drv201_pm_dev_wr(info, NVC_PWR_ON);
		return drv201_param_rd(info, arg, false);
	case NVC_IOCTL_PWR_WR:
		/* This is a Guaranteed Level of Service (GLOS) call */
		pwr = (int)arg * 2;
		dev_info(info->dev, "%s PWR_WR: %d\n", __func__, pwr);
		return drv201_pm_wr(info, pwr);
	case NVC_IOCTL_PWR_RD:
		pwr = info->pwr_dev;
		dev_info(info->dev, "%s PWR_RD: %d\n", __func__, pwr);
		if (copy_to_user((void __user *)arg,
			(const void *)&pwr, sizeof(pwr))) {
			dev_info(info->dev, "%s copy_to_user err line %d\n",
				__func__, __LINE__);
			return -EFAULT;
		}
		return 0;
	default:
		dev_info(info->dev, "%s unsupported ioctl: %x\n", __func__, cmd);
	}
	return -EINVAL;
}
		...
		const struct i2c_device_id *id)
{
	...
	if (info == NULL) {
		dev_info(&client->dev, "%s: kzalloc error\n", __func__);
		return -ENOMEM;
	}
	...
	if (client->dev.platform_data)
		...
		dev_info(info->dev, "%s No platform data. Using defaults.\n",
			__func__);
	}
	...
	if (info->pdata->cfg & (NVC_CFG_NODEV | NVC_CFG_BOOT_INIT)) {
		...
		drv201_pm_wr(info, NVC_PWR_OFF);
		if (err < 0) {
			dev_info(info->dev, "%s device not found\n",
				__func__);
			...
			dev_info(info->dev, "%s device found\n", __func__);
	...
	if (misc_register(&info->miscdev)) {
		dev_info(info->dev, "%s unable to register misc device %s\n",
			__func__, dname);
		drv201_del(info);
		return -ENODEV;
	}
